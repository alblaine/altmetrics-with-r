...
# Step 8. Notice that there are lots of dois with NA valuese. Now write a function to remove all 
...
# Step 16. Graph the data using ggplot2. 
...
ggplot(graph_data) + geom_bar(aes(fct_infreq(factor(cited_by)), times, fill=title_abbr), stat="identity")+
  labs(x = "Media") + scale_fill_manual(values=cbPalette)  + theme_classic() + facet_wrap(~title_abbr)
...
my_graph <- ggplot(graph_data) + geom_bar(aes(fct_infreq(factor(cited_by)), times, fill=title_abbr), stat="identity")+
  labs(x = "Media Types", y="Times Cited", title="Altmetrics for Requested Articles") + scale_fill_manual(values=cbPalette)  + theme_classic() + facet_wrap(~title_abbr)
